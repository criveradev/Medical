...
📖 **[Documentación Swagger](https://api.medical.criveradev.cl/api/v1/docs/)** &nbsp;·&nbsp;
...
**Medical** es un sistema de gestión de citas médicas desarrollado como
...
└─────────────────────┘  WS     └──────────┬───────────┘
                  ...
                  ▼            ▼           ▼            ▼            ▼
            ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐
            │ MongoDB  │ │  Redis   │ │Cloudinary│ │  Resend  │ │  Sentry  │
...
│   ├── src/
│   │   ├── config/                  # env, db, redis, cloudinary, multer, logger, swagger, sentry, email, api (prefijos/versión)
│   │   ├── models/                  # User, Role, Paciente, Doctor, Cita, Historial, Pago, Resultado, Departamento, Especialidad
│   │   ├── controllers/
│   │   │   └── v1/                  # Lógica por recurso (auth, citas, pacientes, doctores, pagos, reportes…)
│   │   ├── routes/
│   │   │   └── v1/                  # Definición de endpoints + validaciones (Swagger en JSDoc)
...
> incluye documentación interactiva completa en Swagger (`/api/v1/docs`).
...
# (Render free, Railway). Si se define, el envío usa la API de Resend en vez de SMTP.
RESEND_API_KEY=
...
La API está versionada en `/api/v1` desde `src/config/api.js` (`API_VERSION`,
`API_PREFIX`). `/api` (sin versión) se mantiene montado como **alias de v1** por
compatibilidad hacia atrás, pero es el mismo alias, no una versión distinta.

En el código, cada versión es un árbol independiente de rutas y controladores
(`controllers/v1`, `routes/v1`), que comparte `models/`, `services/`,
`middleware/` y `config/` con el resto de versiones. `app.js` monta cada
recurso de `v1` en `/api/v1/*` y `/api/*` (legado) desde el mismo router, y
deja documentado en un bloque comentado cómo se agregaría `controllers/v2` /
`routes/v2` el día que un endpoint necesite un cambio incompatible (nuevo
shape de respuesta, validación distinta, etc.) sin romper a los clientes que
siguen en v1.

El frontend (`medical-client/src/lib/api.js`) normaliza toda petición a
`/api/v1/*` en un único punto (`normalizeUrl`), así que consumir una v2 futura
sería cambiar ese archivo, no cada componente.

## 🔌 Endpoints de la API

URL base en producción: `https://api.medical.criveradev.cl/api/v1` · Documentación interactiva: `/api/v1/docs`

> En desarrollo local: `http://localhost:3000/api/v1`
> `/api/...` (sin `/v1`) sigue funcionando como alias de la misma versión.
...
### 8. Email — Resend por API HTTP (importante)

> ⚠️ **Render (plan gratuito) bloquea la salida SMTP** (puertos 25/465/587), así
> que iCloud/Gmail/SMTP tradicional dan *Connection timeout*. La solución es enviar
> por la **API HTTP de Resend** (puerto 443), que el código usa automáticamente
> cuando existe `RESEND_API_KEY`.
...
- [x] Versionado real de la API (`controllers/v1`, `routes/v1` + alias legado)
